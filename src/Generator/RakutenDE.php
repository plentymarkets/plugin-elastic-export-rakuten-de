...
class RakutenDE extends CSVPluginGenerator
{
    ...
    private function buildParentWithoutChildrenRow($item, KeyValue $settings)
    {
        ...
        $data = [
            'id'						=> '',
            'variante_zu_id'			=> '',
            'artikelnummer'				=> $this->elasticExportHelper->generateSku($item['id'], self::RAKUTEN_DE, (int) $settings->get('marketAccountId'), isset($item['data']['skus']['sku']) ? $item['data']['skus']['sku'] : null),
